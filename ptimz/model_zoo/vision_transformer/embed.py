...
def chan_first_reord(dimension):
    new_axis_ord = list(range(2 + dimension))
    chan_axis = new_axis_ord.pop(-1)
    new_axis_ord.insert(1, chan_axis)
    return tuple(new_axis_ord)
...
def resize_pos_embed(pos_embed,
    ...
    src_length = src_shape[0]
    for ss in src_shape[1:]:
        src_length *= ss
    assert L == src_length + num_extra_tokens, \
        f"The length of `pos_embed` ({L}) doesn't match the expected " \
        f'shape ({src_length}+{num_extra_tokens}). Please check the' \
        '`img_size` argument.'
    extra_tokens = pos_embed[:, :num_extra_tokens]

    src_weight = pos_embed[:, num_extra_tokens:]
    src_weight = src_weight.reshape(1, *src_shape, C).permute(*chan_first_reord(len(src_shape)))
