...
default_cfgs = {
    "efficientnetb0_2d\tcmrqa": _cfg(
        url='https://github.com/songphilips/ptimz/releases/download/v0.0.1-hrnet/efficientnetb0_CMR_2d_cmrxmotion22task1.pth.tar',
        input_details='CMR',
        spacing=(2.0, 2.0),
        slice_thickness=8,
        slice_gap=4,
        label_names=['mild motion', 'intermediate motion', 'severe motion'],
        num_classes=3, input_size=(1, 512, 512), last_layer='head_fc'),
}
...
def _build_efficientnet(pretrained_name, arch, dimension, in_chans=None, num_classes=None, **kwargs):
    dimension = dimension.lower()
    assert dimension in ('1d', '2d', '3d'), "dimension must be 1d 2d or 3d"

    if dimension in ['2d']:
        conv_cfg = dict(type=f'Conv{dimension}AdaptivePadding')
    else:
        conv_cfg = dict(type=f'Conv{dimension}')
    norm_cfg = dict(type=f'BN{dimension}', requires_grad=True, eps=1e-3)
    ...
    return build_model_with_cfg(model_cls=EfficientNet, variant='efficientnet', pretrained=pretrained,
                                default_cfg=default_cfgs.get(pretrained_name, None),
                                # model config
                                arch=arch,
                                num_classes=num_classes,
                                in_channels=in_chans, conv_cfg=conv_cfg,
                                norm_cfg=norm_cfg, **kwargs)


@register_model
def efficientnetb0_2d(pretrained=False, **kwargs):
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        pretrained = f'{func_name}\tcmrqa'
    elif isinstance(pretrained, str):
        pretrained = func_name + '\t' + pretrained
        if pretrained not in default_cfgs.keys():
            _logger = logging.getLogger(__name__)
            _logger.warning(f"No {pretrained} pretrained weights, use random initialization.")
            pretrained = False

    return _build_efficientnet(pretrained_name=pretrained, arch='b0', dimension='2d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb0_3d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b0', dimension='3d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb1_2d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b1', dimension='2d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb1_3d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b1', dimension='3d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb2_2d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b2', dimension='2d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb2_3d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b2', dimension='3d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb3_2d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b3', dimension='2d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb3_3d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b3', dimension='3d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb4_2d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b4', dimension='2d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb4_3d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b4', dimension='3d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb5_2d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b5', dimension='2d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb5_3d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b5', dimension='3d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb6_2d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b6', dimension='2d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb6_3d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b6', dimension='3d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb7_2d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b7', dimension='2d', head_type='classification',
                               **kwargs)


@register_model
def efficientnetb7_3d(pretrained=False, **kwargs):
    _logger = logging.getLogger(__name__)
    func_name = str(inspect.stack()[0][3])
    if pretrained is True:
        _logger.warning(f"{func_name} has no pretrained weights, use random initialization.")
    ...
    return _build_efficientnet(pretrained_name=pretrained, arch='b7', dimension='3d', head_type='classification',
                               **kwargs)
