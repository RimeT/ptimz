""" Model Registry
Hacked together by / Copyright timm 2020 Ross Wightman
"""
...
def list_pretrained(default_cfgs):
    pretraines = defaultdict(list)
    for kv in default_cfgs.keys():
        k, v = kv.split('\t')
        pretraines[k].append(v)
    return pretraines
...
def register_model(fn):
    ...
    if hasattr(mod, 'default_cfgs') and model_name in list_pretrained(mod.default_cfgs).keys():
        # this will catch all models that have entrypoint matching cfg key, but miss any aliasing
        # entrypoints or non-matching combos
        pretraines = list_pretrained(mod.default_cfgs)[model_name]
        for pretrain in pretraines:
            cfg_key = model_name + '\t' + pretrain
            cfg = mod.default_cfgs[cfg_key]
            has_valid_pretrained = (
                    ('url' in cfg and 'http' in cfg['url']) or
                    ('file' in cfg and cfg['file'])
                # timm hub id
                # ('hf_hub_id' in cfg and cfg['hf_hub_id'])
            )
            _model_pretrained_cfgs[cfg_key] = mod.default_cfgs[cfg_key]
...
def has_pretrained_cfg_key(model_name, pretrained_name, cfg_key):
    """ Query model default_cfgs for existence of a specific key.
    """
    model_name = model_name + '\t' + pretrained_name
    if model_name in _model_pretrained_cfgs and cfg_key in _model_pretrained_cfgs[model_name]:
        return True
    return False


def is_pretrained_cfg_key(model_name, pretrained_name, cfg_key):
...
def get_pretrained_cfg_value(model_name, pretrained_name, cfg_key):
