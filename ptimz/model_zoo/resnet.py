        ...
        if stride != 1 or in_channels != out_channels:
            ...
            if avg_down and stride != 1:
                ...
                if '2d' in conv_cfg['type']:
                    ...
                    raise ValueError(f'Unknown Conv Type {conv_cfg}')
            downsample.extend([
                build_conv_layer(
                    conv_cfg,
                    in_channels,
...
class ResNet(BaseBackbone):
    ...
    def __init__(self,
        ...
        self._make_stem_layer(in_channels, stem_channels, first_stride)
        ...
        if 'classification' == head_type:
            ...
            self.head_fc = nn.Linear(_out_channels // 2, num_classes)
    ...
    def _make_stem_layer(self, in_channels, stem_channels, stem_stride=None):
        """Make stem layer."""
        stem_stride = 2 if not isinstance(stem_stride, int) else stem_stride
        if self.deep_stem:
            ...
            self.add_module(self.norm1_name, norm1)
            self.relu = nn.ReLU(inplace=True)
        self.maxpool = ConvModule(in_channels=stem_channels,
        ...
        # elif '1d' in self.conv_cfg['type']:
        #     self.maxpool = nn.MaxPool1d(kernel_size=3, stride=2, padding=1)
        # else:
        #     raise ValueError(f"Unknown conv type {self.conv_cfg}")
    ...
    def forward(self, x):
        ...
        if len(outs) == 1:
            if self.head_type is None:
                return outs[0]
            elif 'classification' == self.head_type:
                outs = outs[0]
...
def _build_resnet(pretrained_name, depth, dimension="3d", in_chans=None, num_classes=None, deep_stem=False, **kwargs):
